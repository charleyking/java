...
public class AutoComplete implements DocumentListener {
    ...
    private ArrayList<String> bracketCompletionsList = new ArrayList<>();
    ...
    public AutoComplete(MainFrame frame, ArrayList<String> list) {
        keywordsList = list;
        supportedKeywords = new SupportedKeywords();
        bracketsList = supportedKeywords.getBrackets();
        bracketCompletionsList = supportedKeywords.getBracketCompletions();

        this.frame = frame;

        //Set the handler for the enter key
        InputMap inputMap = frame.textArea.getInputMap();
        ActionMap actionMap = frame.textArea.getActionMap();
    ...
    public void insertUpdate(DocumentEvent e) {
        ...
        int n = Collections.binarySearch(keywordsList, prefix);

        if (n < 0 && -n < keywordsList.size()) {
            String match = keywordsList.get(-n - 1);
    ...
    private void checkForBracket() {
        ...
        for (int i = 0; i < bracketsList.size(); i++) {
            if (bracketsList.get(i).equals(s)) {
                isKeyword = false;
                SwingUtilities.invokeLater(
                        new CompletionTask(bracketCompletionsList.get(i), pos + 1));
            }
        }
    }
    ...
    private class HandleBracketEvent implements KeyListener {
        ...
        public void keyTyped(KeyEvent e) {
            ...
            for (String bracketCompletion : bracketCompletionsList) {
